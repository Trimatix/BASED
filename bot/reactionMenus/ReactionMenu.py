...
class ReactionMenuOption(serializable.Serializable):
    ...
    def __init__(self, name : str, emoji : lib.emojis.BasedEmoji, addFunc : FunctionType = None, addArgs=None, removeFunc : FunctionType = None, removeArgs=None):
        """
...
class CancellableReactionMenu(ReactionMenu):
    ...
    """
    def __init__(self, msg : Message, options : Dict[lib.emojis.BasedEmoji, ReactionMenuOption] = {}, cancelEmoji : lib.emojis.BasedEmoji = cfg.defaultEmojis.cancel,
        ...
        :param options: A dictionary storing all of the menu's options and their behaviour (Default {})
        :type options: dict[lib.emojis.BasedEmoji, ReactionMenuOption]
        :param lib.emojis.BasedEmoji emoji: The emoji members should react with to cancel the menu. (Default cfg.defaultEmojis.cancel)
        :param str titleTxt: The content of the embed title (Default "")
        :param str desc: he content of the embed description; appears at the top below the title (Default "")
        :param discord.Colour col: The colour of the embed's side strip (Default None)
        :param str footerTxt: Secondary description appearing in darker font at the bottom of the embed (Default time until menu expiry if timeout is not None, "" otherwise)
...
class InlineReactionMenu(ReactionMenu):
    ...
    def __init__(self, msg : Message, targetMember : Union[Member, User], timeoutSeconds : int,
                    options : Dict[lib.emojis.BasedEmoji, ReactionMenuOption] = {}, returnTriggers : List[lib.emojis.BasedEmoji] = [], 
    ...
    async def doMenu(self):
        ...
        try:
            reactPL = await botState.client.wait_for("raw_reaction_add", check=self.reactionClosesMenu, timeout=self.timeoutSeconds)
